...
cd Limages
X=imread(strcat(J(1:end-4),'.GIF'));
X=imresize(X,size(inputCXR));
cd ..

cd Rimages
Y=imread(strcat(J(1:end-4),'.GIF'));
Y=imresize(Y,size(inputCXR));
cd ..
T=double(im2bw((X+Y)));
figure,imshow(T);title('Manual Mask ');
%========
S=double(lungMask);
figure,imshow(S);title('Segmented Mask ');
%== Metric caluclation 
